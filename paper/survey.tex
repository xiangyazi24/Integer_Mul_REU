...
The FFT is remarkably well behaved in terms of stability\cite{higham2002accuracy}, but we must have enough bits to ensure that we carry enough precision through the FFT calculations\cite{proakis1995digital}. At a minimum, we need to be able to differentiate twiddle factors, or points on the unit circle. The methods to determine when to split these calculations further to provide smaller blocks of numbers or more precision are quite involved. Interested parties are referred to \cite{brent2010modern} which provides a helpful amount of detail. These details involving the FFT computations for arbitrarily-sized numbers require another $\log{\log{n}}$ term in our big O notation, yielding a final time complexity of $\mathcal{O}(n \log{n} \log{\log{n}})$\cite{higham2002accuracy}\cite{proakis1995digital}\cite{brent2010modern}.
