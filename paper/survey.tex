...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\For{$j \in \{0,1,...,nh-1\}$}
			...
			\State $Y_{chunky}[\lfloor j/h \rfloor] += Y[j] * 10^{j \Mod{h}}$
		\EndFor
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\State $c = 0$
		\For{$j \in \{0,1,...,2n-2\}$}
			\State $c = \lfloor W[j] / B^h \rfloor$				
			\State $W[j] = W[j] \Mod{B^h}$
			\State $W[j+1] += c$
		\EndFor
		\State return $W$
	\end{algorithmic}
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\State $\beta = bd$
		\State $\delta = (a+b)(c+d)$
		\While{$\alpha$, $\beta$, $\delta > $ machine number of digits}
		\State Split each of the $\alpha$, $\beta$, $\delta$ multiplications into $ab$, $cd$ and apply Karatsuba.
		\EndWhile
...
\begin{figure}[H]
	...
	\caption{Recursive Karatsuba Example}
	\label{fig:karatsuba_large_example}
\end{figure}

\subsection{Time Complexity Analysis \nopunct}

As Algorithm~\ref{gs_mult_algo} and Figure~\ref{fig:karatsuba_large_example} show, Karatsuba recursively subdivides the problem in half using three coefficient multiplications per subdivision. The runtime is then  $\mathcal{O}(n^{\log_2{3}})$. $\log_2{3} \approxeq 1.585$, with the runtime complexity of Karatsuba's algorithm commonly referred to as $\mathcal{O}(n^{1.585})$. 

\section{Toom-Cook} \label{toom_cook}

The Toom-Cook algorithm generalizes the divide and conquer approach used in Karatsuba. Instead of splitting the multiplicands into two parts, Toom-Cook splits the multiplication into $r$ parts. Toom-Cook then uses the splits as coefficients in a polynomial, as we have done before. The clever part of Toom-Cook is to use these splits parts in the $X$ and $Y$ coefficients to represent polynomials $x(t)$ and $y(t)$, then multiply those polynomials together. Because the $x(t)$ and $y(t)$ polynomials will be of degree $r-1$, their product will be of degree $2r-2$, at most. We can evaluate the multiplication of $x(t)$ and $y(t)$ at $2r-1$ points to recover the product $w(t)$, of degree $2r-2$ (at most). Hence we are now multiplying together polynomials to recover integers! 
...
So our final answer is $1234 \times 6789 = \num{8377626}$. This operation should look extremely familiar. This is the same as our initial grade school example from section \ref{gs}. We reproduce Figure~\ref{fig:gs01} here for convenience.
\begin{figure}[H]
...
The Fast Fourier Transform algorithm created by Cooley and Tukey in 1965 \cite{cooley1965algorithm} creates a divide-and-conquer method to compute the DFT and take advantage of the symmetry and rotation properties of the complex exponentials used in the DFT. Recall the equation for the DFT:
\begin{equation*}
...
\begin{align*}
	X(m) &= \sum_{n=0}^{N-1}x(n) W_N^{mn} \\
	&= \sum_{n=0}^{N/2-1}x(2n)W_N^{(2n)m} + \sum_{n=0}^{N/2-1}x(2n+1)W_N^{(2n+1)m} \\
	&= \sum_{n=0}^{N/2-1}x(2n)W_N^{2nm} + W_N^m\sum_{n=0}^{N/2-1}x(2n+1)W_N^{2nm}
\end{align*}
...
At this point a graphic may be instructive. We can think of the FFT as a set of recursive equations where we work backward through the DFT summation, halving the computed sum at each step. Figure~\ref{fig:fft_dit_8point} shows how this would look for an 8-point FFT. Decimation in time refers to the fact that we are halving the computed time series at each step - how we separate the summation into $(2n)$ and $(2n+1)$ samples. If we separated the DFT output series into even and odd samples then we would be creating a decimation-in-frequency DFT. Both approaches are used in practice and yield the same result, aside from quantization error. 
\begin{figure}
	\centering
...
If we use modulo arithmetic instead of the complex exponential as our primitive root of unity $g$, then we call the transform a Number-Theoretic Transform (NTT). There are a large number of NTTs used in the multiplication of large integers, typically dealing with prime numbers \cite{baron2003analytical, crandall1999integer, crandall2005prime, harvey2021integer, silverman2014friendly, van1992computational}. The most widely-used NTT for large integer multiplication is Sch\"{o}nhage-Strassen. Here the NTT is sometimes called a Fermat Number Transform (FNT)\footnote{In honor of Fermat's contributions rather than having anything to do with a Fermat number.}.
...
For our example, we will pick a small prime and generator to illustrate the algorithm with our previous example of $\num{1234} \times \num{6789} = \num{8377626}$. We will use the prime $p = 337$ and primitive root of unity/generator $w = 85$. We group single decimal digits for ease, and take the NTT of our input vectors:
