...
\begin{equation}
	...
	X(m) = \sum_{n=0}^{N/2-1}x_{2n}W_{N/2}^{nm} + W_N^m\sum_{n=0}^{N/2-1}x_{2n+1}W_{N/2}^{nm}
\end{equation}
...
\begin{align}
	...
	X(m+N/2) = \sum_{n=0}^{N/2-1}x_{2n}W_{N/2}^{n(m+N/2)} + W_N^{(m+N/2)}\sum_{n=0}^{N/2-1}x_{2n+1}W_{N/2}^{n(m+N/2)}.
\end{align}
As \cite{lyons1997understanding} notes, this looks worse, but we can use our complex exponential properties to reduce the complexity of the equations.
\begin{align}
...
\end{align}
We may now substitute \ref{simplify_twiddle} and \ref{simplify_twiddle} into \ref{complex_second_half_fft} to simplify the second half of the FFT:
\begin{equation}
	\label{second_fft_summation}
	X(m+N/2) = \sum_{n=0}^{N/2-1}x_{2n}W_{N/2}^{nm} - W_N^m\sum_{n=0}^{N/2-1}x_{2n+1}W_{N/2}^{nm}.
\end{equation}
Finally, let's write the equations for the first $N/2$ points \ref{first_fft_summation} and second $N/2$ points \ref{second_fft_summation} together:
\begin{align}
	X(m) &= \sum_{n=0}^{N/2-1}x_{2n}W_{N/2}^{nm} + W_N^m\sum_{n=0}^{N/2-1}x_{2n+1}W_{N/2}^{nm} \\
	X(m+N/2) &= \sum_{n=0}^{N/2-1}x_{2n}W_{N/2}^{nm} - W_N^m\sum_{n=0}^{N/2-1}x_{2n+1}W_{N/2}^{nm}.
\end{align}
