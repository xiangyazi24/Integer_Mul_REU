...
\section{Karatsuba}
We now turn our attention to Karatsuba's algorithm. In 1960, Andrey Kolmogorov conjectured that $\mathscr{O}(n^2)$ was as fast as multiplication could be achieved. Kolmogorov held a seminar at Moscow University where he presented this and other conjectures. Anatoly Karatsuba, a 23-year-old student, found a divide and conquer method that multiplies numbers in $\mathscr{O}(n^{\log_2 3})$ that he presented at the next seminar, which was then disbanded (TODO: INSERT REFERENCE HERE). Perhaps the best way to motivate some is to say that something is impossible.
...
Figure~\ref{fig:karatsuba_example} shows an example for $25786109 \times 72166948 = 1860904787325332$ with the same coloring used to denote $abcd$ as the previous explanations. Note that we have combined the algorithm with chunking (typically this is referred to as blocking, but the term chunking is more fun) that combines several coefficients. 
...
Karatsuba's three coefficient products, $ac$, $bd$, and $(a+b)(c+d)$, can be used to recursively subdivide the problem further.

\subsection{Time Complexity Analysis \nopunct}

As we noted above, because Karatsuba allows recursively subdividing the problem using coefficient products, the runtime for products is $\mathscr{O}(n^{\log_2{3}})$. $\log_2{3} \approxeq 1.58$, so the runtime complexity of Karatsuba's algorithm is commonly referred to as $\mathscr{O}(n^{1.58})$. 
