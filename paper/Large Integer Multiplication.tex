...
As we noted above, because Karatsuba allows recursively subdividing the problem using coefficient products, the runtime for products is $\mathscr{O}(n^{\log_2{3}})$. $\log_2{3} \approxeq 1.585$, so the runtime complexity of Karatsuba's algorithm is commonly referred to as $\mathscr{O}(n^{1.585})$. 
