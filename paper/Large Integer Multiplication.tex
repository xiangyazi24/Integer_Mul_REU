...
The title of the paper is a tribute to Dr. Silverman's wonderful \textit{A Friendly Introduction to Number Theory}. We will start with grade school multiplication, then cover the first advance in nearly 4000 years (TODO: Ref History of Mathematics), Karatsuba's algorithm. Next we will discuss the Toom Cook algorithm that is a variation on Karatsuba's algorithm. Then we turn to the DFT and FFT as a foundation to understand how to multiply numbers using the algorithms. Finally, we cover the Sch\"{o}nhageStrassen multiplication algorithm, which gets us to $\mathcal{O}(n \log n \log \log n)$ time complexity.

\section{Grade School Multiplication}
Multiplication dates back to at least 2000 B.C. (TODO: Ref History of Mathematics) and of course we all learn how to multiply integers in grade school. A review of this algorithm gives us a grasp of the terminology and notation we will use throughout the rest of this paper to discuss multiplication algorithms. We start with a representation for integers, which will be useful for later algorithms.

\subsection{Integer Representation \nopunct}

We can express any positive integer $x$ as a summation of coefficients raised to a base power:
\begin{equation}
...
\begin{minted}{python}
	def pad_list(lst, length, pad=0):
	"""Pad the list to the given length with the pad value."""
	...
	X = pad_list(X, max_length)
	Y = pad_list(Y, max_length)
	
	# Initialize the result list W with zeros, length should be 2 * max_length
	W = [0] * (2 * max_length)
	
	# Perform the multiplication using a double for loop
	for j in range(len(X)):
	for k in range(len(Y)):
	W[j + k] += X[j] * Y[k]
	
	# Handle carries from the least significant digit to the most
